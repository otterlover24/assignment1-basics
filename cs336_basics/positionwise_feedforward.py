...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gate_output = self.gate_proj(x) # W_1 @ X
        up_output = self.up_proj(x) # W_3 @ X

        # Apply the SiLU activation and element-wise multiplication
        fused_output = self.activation(gate_output) * up_output # SiLU(W_1 @ X) elem* (W_3 @ X)

        # Apply the final down projection
        output = self.down_proj(fused_output)   # W_2 @ ( SiLU(W_1 @ X) elem* (W_3 @ X) )
