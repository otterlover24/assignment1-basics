...
def run_train_bpe(
    ...
    **kwargs,
    ...
    # 1. Setup
    # purpose: for ease of visualization
    ...
    for i in tqdm(range(num_merges), desc="BPE Merges"):
        ...
        for word in words_to_update:
            ...
            ## END OLD
            for i, matching_best_pair in enumerate(new_word_tuple):
                if matching_best_pair == best_pair:
                    if i - 1 >= 0:
                        pair_stats[new_word_tuple[i - 1], new_word_tuple[i]] += count
                    if i + 1 < len(new_word_tuple):
                        pair_stats[new_word_tuple[i], new_word_tuple[i + 1]] += count
